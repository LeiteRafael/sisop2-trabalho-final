...
char manager_ip[MAX_MSG_LEN];
char manager_port[MAX_MSG_LEN];
int known_manager = 0;

char guest_ip[MAX_MSG_LEN];
char guest_port[MAX_MSG_LEN];
char guest_mac[MAX_MSG_LEN];
int known_guest = 0;
...
void *managerMonitoringService(void *socket) {
    ...
    monitoring_addr.sin_port = htons(atoi(guest_port));
    monitoring_addr.sin_addr.s_addr = inet_addr(guest_ip);

    /*memset(&monitoring_addr, 0, sizeof(monitoring_addr));
    monitoring_addr.sin_family = AF_INET;
	monitoring_addr.sin_addr.s_addr = INADDR_ANY;
	monitoring_addr.sin_port = htons(MONITORING_PORT);*/

    printf("enviando SSR para %s:%d\n", inet_ntoa(((struct sockaddr_in *) &monitoring_addr)->sin_addr), ntohs(monitoring_addr.sin_port));

    while (1) {
        printf("enviou SSR para guest");
...
void *guestMonitoringService(void *socket) {
    ...
    memset(&monitoring_addr, 0, sizeof(monitoring_addr));
    monitoring_addr.sin_family = AF_INET;
	monitoring_addr.sin_addr.s_addr = inet_addr(manager_ip);
	monitoring_addr.sin_port = htons(MONITORING_PORT);
...
void *discoveryService(void *socket) {
    ...
    socklen_t d_addr_len = sizeof(discovery_addr);

    while (1) {
        int bytes_received = recvfrom(discovery_socket, discovery_buf, sizeof(discovery_buf), 0, (struct sockaddr *) &discovery_addr, &d_addr_len);
        if (bytes_received > 0) {
            printf("%s:%d entrou: \"%s\"\n", inet_ntoa(((struct sockaddr_in *) &discovery_addr)->sin_addr), ntohs(discovery_addr.sin_port), discovery_buf);
            sprintf(guest_ip, "%s", inet_ntoa(((struct sockaddr_in *) &discovery_addr)->sin_addr));
            sprintf(guest_port, "%d", ntohs(discovery_addr.sin_port));
            sprintf(guest_mac, "%s", discovery_buf);
            known_guest = 1;
...
int main(int argc, char *argv[]) {
    ...
    struct sockaddr_in monitoring_addr;
    socklen_t m_addr_len = sizeof(monitoring_addr);
    ...
    if(strcmp(argv[1], "manager") == 0){
        ...
        while (!known_manager) {
            int bytes_received = recvfrom(discovery_socket, discovery_msg, MAX_MSG_LEN, 0, (struct sockaddr *) &discovery_addr, &d_addr_len);

            if (bytes_received > 0) {
                printf("De %s:%d: %s\n", inet_ntoa(((struct sockaddr_in *) &discovery_addr)->sin_addr), ntohs(discovery_addr.sin_port), discovery_msg);
                sprintf(manager_ip, "%s", inet_ntoa(((struct sockaddr_in *) &discovery_addr)->sin_addr));
                sprintf(manager_port, "%d", ntohs(discovery_addr.sin_port));
                known_manager = 1;
                break;
            }
        }
        ...
        guestMonitoringService(&monitoring_socket);
